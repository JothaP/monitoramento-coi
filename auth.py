...
def obter_cookie():
    try:
        ...
        # Força o componente a sincronizar os cookies
        # do navegador com o Streamlit.
        controller.getAll()
...
def verificar_autenticacao():
    ...
    try:
        # Na abertura da página/F5, o componente precisa
        # de um pequeno intervalo para devolver os cookies.
        controller.getAll()
    except Exception:
        pass
    ...
    tentativa = st.session_state.get(
        "_tentativa_restauracao_login",
        0,
    )

    if tentativa < 2:
        st.session_state["_tentativa_restauracao_login"] = tentativa + 1

        time.sleep(1)

        st.rerun()

    st.session_state["_tentativa_restauracao_login"] = 0
